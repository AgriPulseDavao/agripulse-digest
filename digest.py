...
Runs unattended (weekly via GitHub Actions). Config comes from env vars:
  SUPABASE_URL    e.g. https://hewpdiuhxdzayihgcncw.supabase.co
  SUPABASE_KEY    Supabase secret key (sb_secret_...). Server-side only.
  RESEND_API_KEY  Resend API key (re_...)
  MAIL_FROM       sender, default "AgriPulse <onboarding@resend.dev>"
...
MAIL_FROM = os.environ.get("MAIL_FROM", "AgriPulse <onboarding@resend.dev>")
MAIL_TO = os.environ["MAIL_TO"]
WINDOW_DAYS = int(os.environ.get("WINDOW_DAYS", "30"))
TZ_OFFSET_H = 8  # Davao / Asia-Manila, UTC+8

WEIGHTS = {"ec": 20, "ph": 20, "panama": 15, "sigatoka": 15, "moisture": 15, "sensors": 15}
LABELS = {"ec": "Soil EC (nutrients)", "ph": "Soil pH", "panama": "Panama disease risk",
          "sigatoka": "Sigatoka disease risk", "moisture": "Soil moisture", "sensors": "Sensor health"}
TARGETS = {"ec": "200-800 uS/cm", "ph": "5.0-7.5", "panama": "Risk = LOW",
           "sigatoka": "Risk = LOW", "moisture": "30-85%", "sensors": "All sensors reporting"}
...
def compute(series, latest_status, last_seen):
    ...
    f["sensors"] = dict(score=round(100 * reporting / 3), n=reporting, of=3)
    tw = sw = 0
...
def render_html(composite, f, trend, generated):
    g = grade(composite)
    acts = []
    for k, d in f.items():
    ...
    .head{{border-bottom:3px solid #1f7a3d;padding-bottom:10px;margin-bottom:16px}}
    .head h1{{margin:0;color:#15592c;font-size:20px}} .sub{{color:#637067;font-size:11px}}
    .scorebox{{text-align:center;border:1px solid #e3e9e2;border-radius:10px;padding:14px;margin-bottom:14px}}
    .big{{font-size:54px;font-weight:800;color:{g[1]}}} .grade{{display:inline-block;color:#fff;background:{g[1]};padding:3px 14px;border-radius:20px;font-weight:800;font-size:12px}}
    h2{{color:#15592c;font-size:13px;text-transform:uppercase;letter-spacing:.04em;margin:18px 0 8px;border-bottom:1px solid #e3e9e2;padding-bottom:4px}}
    ...
    .factor{{margin-bottom:11px}} .ftop{{display:flex;justify-content:space-between;font-size:12px}}
    ...
    <div class='note'>Auto-generated from your Supabase sensor backup. Factors with no recent data are excluded from the score.</div>
    </body></html>"""
...
def main():
    ...
    html = render_html(composite, f, trend, generated)
